...
uint32_t MyQueue::EnQueue(char *str, int len)
{
    ...
    else
    {
        while ( true)
        {
             ...
             if (elements[next] == '\n')
             {
                 ...
                 {
                    memcpy(buff, elements + first + 1, QUEUE_AVAILABLE_MAX - first);
                    memcpy(buff + QUEUE_AVAILABLE_MAX - first, elements, next + 1);
                 }
                 first = next;
                 return true;
             }
...
void print_queue(MyQueue _queue)
{
    ...
    printf("==>first:last:available:count = %d : %d : %d : %d\n", _queue.first, _queue.last, _queue.Available(), _queue.Count());
    printf("\n");
}
